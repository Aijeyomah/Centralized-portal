...
const AssessmentHistory = (props) => {
    ...
    useEffect(() => {
        ...
        axios.get("/api/v1/getassessmentbyadmin", config)
            .then(res => {
    ...
    const handleNext = (e) => {
        ...
        if (index <= assessment.length - 1) {
            ...
            setState({
                ...
                answer: assessment[index + 1].correct_answer,
            ...
            {batchinfo.length !== 0 ?
                <div className='history_wrapper'>
                    ...
                    <table className='assessmentHistory_table'>
                        ...
                        <tbody>
                            {[...batchinfo].map((e, id) => (
                                <tr key={e.id} className='histories_tr'>
                                    <td className='assessmentHistory_batch'>Batch {e.batch}</td>
                                    <td>{e.date}</td>
                                    <td>{e.question_no} </td>
                                    <td>{e.time}</td>
                                    <td>{e.status}</td>
                ...
                : <div>There are no applications to review</div>
            }
        </div>
    );
